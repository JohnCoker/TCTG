  ...
  describe('getUnit', function() {
    describe('default', function() {
      it("choice must be unset", function() {
        expect(window.localStorage.getItem('units')).toBe(null);
      });
      it("length must be mm", function() {
        var u = getUnit('length');
        expect(u).toBeDefined();
        expect(typeof u).toBe('object');
        expect(u.label).toBe('mm');
        expect(u.toMKS).toBe(.001);
      });
    ...
    describe('custom', function() {
      beforeEach(function() {
        var std = UnitDefaults.get('in/lb'),
            custom = std.copy();
        custom.force = 'lbf';
        custom.altitude = 'mi';
        custom.label = 'custom';
        custom.defaults = false;
        Units.setDefaults(custom);
      });
      it("choice must be set", function() {
        var choice = Units.getDefaults();
        expect(choice).toBeDefined();
        expect(choice.label).toBe('custom');
        expect(choice.defaults).toBe(false);
      });
      it("length must be in", function() {
        var u = getUnit('length');
        expect(u).toBeDefined();
